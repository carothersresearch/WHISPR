...
file_name = "C:/Users/rycar/Documents/GitHub/WHISPR/inputs/211117_ABA_titration.xlsx"
...
output_plate = pd.read_excel(file_name, sheet_name = "plate_layout_2", index_col = 0, dtype = str)

#CSV file with each reaction as rows and the volume of each input (columns) added to reach final reaction volume
mixing_table_df = pd.read_excel(file_name, sheet_name = "mixing_table_2", index_col = 0, dtype = str).fillna(0)
...
vol_table_df = generateVolumeTable(mixing_table_df, source_plate_df,rxn_vol = 2.25, total_vol = 10)

output_df = writeProtocol(source_plate_type, vol_table_df, source_plate_df, output_plate,source_plate_df)
filename = '211117_aba_2.csv'
